...
// TODO:ABI bytecode in js
    ...
    function get_borrower(address addr)public view returns (string memory, string memory,string memory,string memory,uint256,uint256,string memory ){
        ...
        return (borowerInfo[addr].name, borowerInfo[addr].aadhar, borowerInfo[addr].pan, borowerInfo[addr].email, borowerInfo[addr].score,borowerInfo[addr].loan_amountt,borowerInfo[addr].l_purpose);
    }
    
    function loan_data(address addr)public view returns (string memory, string memory,uint256,string memory,uint256 ){
        // if(keccak256(patientInfo[addr].name) == keccak256(""))revert();
        return (borowerInfo[addr].name, borowerInfo[addr].email, borowerInfo[addr].phone, borowerInfo[addr].l_purpose, borowerInfo[addr].loan_amountt);
    }
